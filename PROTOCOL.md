...
Status: frozen as of the freeze commit. This document is the referee for Phases 3 and 4, and the git commit that introduces this line is the pre-registration record. Everything before the freeze commit is protocol formation and repair; everything after it is execution against frozen rules. After the freeze commit, changes are recorded only as dated entries in the companion document AMENDMENTS.md, with a rationale; this file is never edited. Where implementation and this document disagree, the disagreement is a validation finding, not a choice.

Passages marked "Adopted before the freeze commit" were settled during protocol formation, several of them after an independent audit returned FAIL against an earlier draft and the repair that followed. They are protocol text here rather than a change log: the dated record of how each was reached lives in the git history the freeze commit closes, and AMENDMENTS.md holds only what changes after this point.
...
Parallax bins: the adopted edges (0.025, 0.05, 0.1, 0.2, 0.4, overflow) live in the same config, plus the zero bin for exact-zero-parallax pairs. For translation-program pairs, the open interval from 0 to the program's design floor is asserted empty, and the assertion is enforced rather than silently absorbed by a bin; orbit pairs may legitimately fall in that interval, and the (0, 0.025) bin exists for them in the joint analysis. Bin intervals are closed on the right: a value equal to an edge belongs to the lower bin, matching the implementation and frozen here.

Adopted before the freeze commit. Three of these values decide which pairs are drawn or gated and are therefore part of what a run measured, not of how it is read. The strata the pair sample is drawn within have their own frozen edges, stratum_parallax_edges and stratum_rotation_edges_deg, separate from the reporting bins above even though they hold the same numbers: the reporting edges may be widened once from counts after the run, and widening a label must not retroactively redefine which pairs a later scene contributes. The right-closed convention governs those stratum labels as well as the reporting bins, so it is fixed here and no post-run edit to it is permitted. The translation programme's design floor is its own value, translation_parallax_design_floor, and not the first reporting edge, so that a permitted widening of that edge cannot move an evaluation-time assertion. All three belong to the measurement identity defined in 3.12.
...
Adopted before the freeze commit. The estimand of a reported cell is the unweighted mean over camera pairs of that cell's pair-level values. This section fixes the unit twice already, in making support depend on independent camera pairs and scene coverage rather than raw comparison counts, and in excluding points and patches from resampling by name; weighting a pair by how many of its correspondences survived would make the point the unit of the point estimate while the interval around it kept the pair, so the two would describe different quantities. The comparison-weighted value is reported beside it as a diagnostic column, because the weighting is not neutral: a pair's comparison count is largely set by how much of the target the context still sees, so within a bin the weight rises with the easier geometry. Every reported interval also carries the number of bootstrap replicates that produced it, since a cell whose statistic is undefined in some replicates yields a quantile over the draws in which it existed, and a quantile over three draws and one over a thousand otherwise print identically.
...
Resolution of the verification note, adopted before the freeze commit. An earlier draft recorded that the completed run reported a Mean-Feature floor differing by path and finite centered Mean-Feature values, both impossible under this section's definitions. Validation identified the cause: the implementation used a position-conditioned mean map on one path and a global vector on the other, and centered against a different object again. Mean-Feature is one global D-dimensional vector, the same object serves as the centering statistic, and its centered cosine is not applicable; the Phase 3 aggregates were re-run under these definitions. The two paths still differ in what they score their prediction against, a sampled read on one and a pooled cell on the other, so their Mean-Feature scores are not required to be equal and equality is not asserted.
...
The gated statistic is the signed aggregate: the unweighted mean over pairs of the per-pair difference between the two paths, for raw and centered cosine alike. The tolerance is 0.003, calibrated from the pilot run's aggregate path agreement and retained as the operator gate. It is not re-derived from the effects it exists to be independent of, and it is not scoped per encoder. The per-pair absolute difference, its median, its maximum, and the count of pairs above tolerance are reported beside the gate as diagnostics and are not gated: dispersion between two unbiased operators is variance, not bias, and it is judged by the path-agreement ledger rather than by a bound calibrated on an aggregate.

Where an interpreted effect is no larger than this tolerance, the tolerance does not certify it. Such an effect is reported under both evaluation paths, with both estimates and their difference shown, and its interpretation is restricted to content the two paths share. Both estimates within the band with one sign and both intervals clear of zero licenses a claim of a small, sign-consistent effect; exactly one within the band licenses the effect but not a claim about its size, which is reported as path-sensitive; a difference in sign, or an interval that includes zero, licenses no claim of advantage and the effect is reported as being at the scale of evaluation-path choice. Every term of such a quantity is recomputed on the cross-path common-valid cell set before differencing, and its uncertainty comes from a paired scene bootstrap in which one draw of scenes serves both paths and the difference is recomputed inside each replicate.
...
### 3.12 Constants, identity, and provenance, frozen before Phase 4

Adopted before the freeze commit. This section fixes what a run's identity is, so that two runs can be compared only when they measured the same thing.

The analysis config carries two identities. The measurement identity covers every value that decides what the rows contain: the co-visibility tolerance, the minimum co-visible fraction, the points sampled per pair, the per-stratum cap, the sampling-design edges and the right-closed convention of 3.4, the translation design floor, the manifest bounds below, and the frame filters. A report may not be built from a run whose measurement identity differs from the config reading it. The reporting identity covers bin edges, support thresholds, bootstrap settings, and gate tolerances; a difference there is reported and permitted, because 3.4 allows those to be set from realized counts after the run.

Two geometric bounds are held consistent with the tolerances they interact with, and neither may be set alone. The in-place rotation programme's frames share a camera position to within rotation_position_bound_m; that bound divided by the shallowest median depth the frame filter admits must not exceed zero_parallax_tol, or a pure-rotation pair would leave the zero-parallax bin its regime is defined by. The translation programme's frames share one orientation to within translation_rotation_bound_deg, measured over every unordered pair rather than against a reference frame, and that bound must not exceed zero_rotation_tol_deg. Rotation magnitude is computed from the skew term against the trace term, not from the trace alone, because the latter cannot resolve angles below the zero-rotation tolerance and would make that bin a statement about arithmetic.

The encoders are locked, and locked means retrievable. Each cached feature set records the weights fingerprint, the weights revision, the inference-code revision, and a content digest of the arrays themselves; the evaluation run record carries all of these forward per scene. A revision is a full commit hash. A branch or tag name is a moving reference and is not a pin. Where a distribution makes a checkpoint unpinnable, as an unversioned download URL does, that impossibility is declared explicitly and the fingerprint is the whole of the evidence that the bytes did not change; an unset pin is not such a declaration. A report may not be built from provenance that is absent, incomplete, or unpinned, and every scene of a run must share one encoder identity.
...
Changes after the freeze commit are recorded in the companion document AMENDMENTS.md, which is normative and takes precedence over this document wherever the two differ. This file is never edited after the freeze commit. Passages above marked "Adopted before the freeze commit" were settled during protocol formation and are protocol text rather than amendments; AMENDMENTS.md holds only what changes after the freeze.